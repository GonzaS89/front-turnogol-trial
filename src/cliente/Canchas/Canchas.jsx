...
<p className="mt-2 text-sm sm:text-base text-gray-600">
  ...
  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-transform duration-300 group-focus-within:scale-110 group-focus-within:text-emerald-600">
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-400 group-focus-within:text-emerald-600 transition-colors duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  />
      ...
      {!showResults && (
        <div className="flex flex-col items-center justify-center gap-4 p-8 bg-white/70 backdrop-blur-sm rounded-xl shadow-md text-center max-w-md mx-auto mt-6">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-16 h-16 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <h3 className="text-lg md:text-xl font-semibold text-gray-800">Empieza a buscar</h3>
      ...
      {isLoading ? (
        <div className="w-full h-40 sm:h-48 flex flex-col justify-center items-center bg-white/70 backdrop-blur-sm rounded-xl shadow-inner">
          ...
          <p className="text-xs sm:text-sm md:text-base font-medium text-gray-600">Cargando canchas...</p>
        </div>
      ) : error ? (
        ...
        <div className="mt-6 space-y-6 w-full lg:max-w-5xl xl:max-w-7xl">
          {filteredCanchas.length === 0 ? (
            ...
            {/* Título */}
            <h3 className="text-xl md:text-2xl font-bold text-gray-800">No encontramos canchas</h3>
